...
// RCS-ID:      $Id: main_listctrl_[most_of_get_set_data_approach].cpp,v 1.1 2002/06/01 18:31:32 robertoconnor Exp $
...
{   
    ...
    for ( ;; ) {
        ...
        // Use our function to get the row's channel section
        section_string = get_row_channel_section( selected_row_number );
...
{    
    ...
    // Get the number of the last row, while inserting the info into the 3 columns
    newly_inserted_row_number = InsertItem( cell_info );   
    SetItem( newly_inserted_row_number, DUE_COLUMN, due_string );     
    SetItem( newly_inserted_row_number, SECTION_COLUMN, channel_section );
}
...
/*
    ...
    channel_section = get_cell_contents_string( row_number, SECTION_COLUMN );
...
*/
    ...
    data = GetItemData( row_number );
    
    channel_section = ( *( (wxString*)data ) );
...
// Sets the wxString of a channel's section in the config, based on its row number
void main_listctrl::set_row_channel_section( long row_number, 
                                             const wxString& channel_section )
{
    wxString*   channel_section_to_cast;    
    long        data;
    
    channel_section_to_cast = &channel_section;
        
    SetItemData( row_number 
}


// integer increaser
#include <iostream.h> void increase (void* data, int type)
{
  switch (type)
  {
...
{
    set_column_widths();
    // Allow this event to be processed by a higher handler.
    event.Skip();
}
...
void main_listctrl::set_column_widths()
{
    ...
    name_column_width -= GetColumnWidth( SECTION_COLUMN );
