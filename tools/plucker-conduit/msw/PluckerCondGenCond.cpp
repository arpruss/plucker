...
#include "PluckerCondGenCond.h"
//#include "PluckerCondSync.h"
#include "resource.h"
...
//     Function:        DllMain()
//
//     Description:    main entry point to the PluckerCond component
//
...
ExportFunc long OpenConduit(PROGRESSFN pFn, CSyncProperties& rProps)
{
    ...
    if (rProps.m_SyncType == eDoNothing) {
        LogAddEntry("OK Plucker - configured to do nothing", slSyncFinished, false);
        return retval;
    }

    // get location of ini file
    HKEY iniKey;
    LONG err = RegOpenKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\The Plucker Team\\Plucker", 0, KEY_READ, &iniKey);
    if (err != ERROR_SUCCESS) {
        LogAddEntry("Plucker - could not find registry key for plucker.ini (has Plucker Desktop been installed?), update aborted.", slSyncAborted, false);
        return 1;
    }
    DWORD pluckerPathSize = 1024;
    DWORD type = REG_SZ;
    char pluckerPath[1024];
    err = RegQueryValueEx(iniKey, "Path", NULL, &type, (LPBYTE) pluckerPath, &pluckerPathSize);
    if (err != ERROR_SUCCESS) {
        LogAddEntry("Plucker - could not find registry key for plucker.ini path (has Plucker Desktop been installed?), update aborted.", slSyncAborted, false);
        return 1;
    }
    RegCloseKey(iniKey);
    char iniFileName[1048];
    strcpy(iniFileName, pluckerPath);
    strcat(iniFileName, "plucker.ini");

    // see if we are supposed to run
    int autoUpdateMode = GetPrivateProfileInt("PLUCKER_DAEMON", "autoupdate_mode", 0, iniFileName);
    if (autoUpdateMode == 0) {
        LogAddEntry("Plucker - update turned off in Plucker Desktop", slSyncFinished, false);
        return retval;
    }

    // find command line to run
    char defaultCommand[2048];
    strcpy(defaultCommand, pluckerPath);
    strcat(defaultCommand, "plucker_desktop\\plucker-desktop.exe --update-due");

    char commandLine[2048];
    GetPrivateProfileString("PLUCKER_DAEMON", "sync_commandline", defaultCommand, commandLine, 2048, iniFileName);
    // write current user
    WritePrivateProfileString("PLUCKER_DAEMON", "username_filter", rProps.m_UserName, iniFileName);

    if (strlen(commandLine) <= 0) {
        strcpy(commandLine, defaultCommand);
    }
    ...
    CreateProcess(NULL, 
            ...
            FALSE,
            CREATE_DEFAULT_ERROR_MODE | CREATE_NEW_PROCESS_GROUP | CREATE_NEW_CONSOLE, //0,             // creation flags 
    ...
    //HANDLE handles[1];
    //handles[0] = piProcInfo.hProcess;
    //readErr = MsgWaitForMultipleObjects(1, handles, TRUE, 500, QS_ALLEVENTS);
    readErr = WaitForSingleObject(piProcInfo.hProcess, 1000);
    while (readErr == WAIT_TIMEOUT) {
        SyncYieldCycles(100);
        //readErr = MsgWaitForMultipleObjects(1, handles, TRUE, 500, QS_ALLEVENTS);
        readErr = WaitForSingleObject(piProcInfo.hProcess, 1000);
    }
	...
	LogAddFormattedEntry(slSyncFinished, false, "Plucker for %s", commandLine);
    ...
    switch (Message) {
        case WM_INITDIALOG:
            ...
            if (lParam != 0) {
                ...
                if (pCfgInfo->syncTemporary == eHHtoPC || pCfgInfo->syncTemporary == eFast || pCfgInfo->syncTemporary == eSlow) {
                    pCfgInfo->syncTemporary = ePCtoHH;
                }
